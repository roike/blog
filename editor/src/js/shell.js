...
const setDomMap = () => {
  ...
  domMap.account = document.getElementById("shell-head-acct");
};
...
const onLogin = (event) => {
  ...
  spa.uriAnchor.setAnchor({ page: user_map.anchor||'heading'}, false);
};

const onLogout = () => {
  model.user.logout();
  spa.uriAnchor.setAnchor({ page: "login" }, false);
};
...
const onPopstate = () => {
  ...
  let anchor = anchor_map_proposed.page[0];
  const user = model.user.get();
  const classlist = domMap.account.classList;
  if (user) {
    classlist.replace("logout", "login");
    //spa.uriAnchor.setBreadCrumb();
  } else {
    classlist.replace("login", "logout");
    anchor = "login";
  }
...
const init = () => {
  ...
  //グローバルカスタムイベントのバインド
  spa.gevent.subscribe("shell", "login", onLogin);
  spa.gevent.subscribe("shell", "logout", onLogout);
  spa.gevent.subscribe("shell", "error", onError);
  spa.gevent.subscribe("shell", "notify", onNotify);

  // ローカルイベントのバインド
  document.addEventListener("click", handleAnchorClick);
  //Stop when the submit is fired 基本submit は使わないので無効にしておく
  domMap.container.addEventListener("submit", (e) => e.preventDefault());
